...
router.post('/vendors', auth_1.requireWriter, async (req, res) => {
    try {
        const validation = (0, validators_1.validateVendor)(req.body);
...
router.patch('/vendors/:id', auth_1.requireWriter, async (req, res) => {
    try {
        const vendor = await vendorRepo.findById(req.params.id);
        if (!vendor) {
            res.status(404).json({ error: 'Vendor not found' });
        ...
        const validation = (0, validators_1.validateVendor)(updated);
        if (!validation.isValid()) {
            res.status(400).json(validation.toResponse());
            return;
        }
...
router.delete('/vendors/:id', auth_1.requireAdmin, async (req, res) => {
    try {
        const vendor = await vendorRepo.findById(req.params.id);
        if (!vendor) {
            res.status(404).json({ error: 'Vendor not found' });
...
router.post('/vendors/:id/deactivate', auth_1.requireWriter, async (req, res) => {
    try {
        const vendor = await vendorRepo.findById(req.params.id);
        if (!vendor) {
            res.status(404).json({ error: 'Vendor not found' });
...
router.post('/contracts', auth_1.requireWriter, async (req, res) => {
    try {
        const validation = (0, validators_1.validateContract)(req.body);
        if (!validation.isValid()) {
            res.status(400).json(validation.toResponse());
            return;
        }
...
router.patch('/contracts/:id', auth_1.requireWriter, async (req, res) => {
    try {
        const contract = await contractRepo.findById(req.params.id);
        if (!contract) {
            res.status(404).json({ error: 'Contract not found' });
        ...
        const validation = (0, validators_1.validateContract)(updated);
        if (!validation.isValid()) {
            res.status(400).json(validation.toResponse());
            return;
        }
...
router.delete('/contracts/:id', auth_1.requireWriter, async (req, res) => {
    try {
        const contract = await contractRepo.findById(req.params.id);
        if (!contract) {
            res.status(404).json({ error: 'Contract not found' });
...
router.post('/contracts/:id/archive', auth_1.requireWriter, async (req, res) => {
    try {
        const contract = await contractRepo.findById(req.params.id);
        if (!contract) {
            res.status(404).json({ error: 'Contract not found' });
...
router.post('/contracts/:id/restore', auth_1.requireAdmin, async (req, res) => {
    try {
        const contract = await contractRepo.findById(req.params.id);
        if (!contract) {
            res.status(404).json({ error: 'Contract not found' });
...
router.get('/contracts/:contractId/reminders', async (req, res) => {
    try {
        const reminders = await reminderRepo.findByContract(req.params.contractId);
...
router.post('/contracts/:contractId/reminders', auth_1.requireWriter, async (req, res) => {
    try {
        const validation = (0, validators_1.validateReminder)({ ...req.body, contract_id: req.params.contractId });
        if (!validation.isValid()) {
            res.status(400).json(validation.toResponse());
            return;
        }
...
router.patch('/reminders/:id', auth_1.requireWriter, async (req, res) => {
    try {
        const reminder = await reminderRepo.findById(req.params.id);
        if (!reminder) {
...
router.get('/contracts/export.csv', async (req, res) => {
    try {
